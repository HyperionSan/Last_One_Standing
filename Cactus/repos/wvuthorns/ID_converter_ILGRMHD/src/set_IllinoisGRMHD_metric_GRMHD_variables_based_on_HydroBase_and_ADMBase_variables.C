...
#pragma omp parallel for
    for(int k=0;k<cctk_lsh[2];k++) for(int j=0;j<cctk_lsh[1];j++) for(int i=0;i<cctk_lsh[0];i++) {
          ...
          Avec[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,0)]=0;
          Avec[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,1)]=0;
          Avec[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,2)]=0;
          Aphi[index]=0;
        }
  }
...
#pragma omp parallel for
  for(int k=0;k<cctk_lsh[2];k++) for(int j=0;j<cctk_lsh[1];j++) for(int i=0;i<cctk_lsh[0];i++) {
	...
	Ax[index] = Avec[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,0)];
	Ay[index] = Avec[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,1)];
	Az[index] = Avec[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,2)];
	psi6phi[index] = Aphi[index];

	double ETvx = vel[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,0)];
	double ETvy = vel[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,1)];
	double ETvz = vel[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,2)];
