...
#pragma omp parallel for
  for(int k=0;k<cctk_lsh[2];k++) for(int j=0;j<cctk_lsh[1];j++) for(int i=0;i<cctk_lsh[0];i++) {
	...
	CCTK_REAL ETvx = vel[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,0)];
	CCTK_REAL ETvy = vel[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,1)];
	CCTK_REAL ETvz = vel[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,2)];
