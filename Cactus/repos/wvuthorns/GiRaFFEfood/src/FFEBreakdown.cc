  ...
  for(int k=kmin;k<kmax;k++)
    for(int j=jmin;j<jmax;j++)
      for(int i=imin;i<imax;i++) {
        ...
        Avec[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,0)] = 0.0;
        Avec[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,1)] = AyL;
        Avec[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,2)] = AzL;
...
// Equation (99), arXiv:1310.3274v2. Calculate the velocity.
        ...
        vel[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,0)] = (EyL*BzL - EzL*ByL)/B2L;
        vel[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,1)] = (EzL*BxL - ExL*BzL)/B2L;
        vel[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,2)] = (ExL*ByL - EyL*BxL)/B2L;
