  ...
  for(int k=kmin;k<kmax;k++)
    for(int j=jmin;j<jmax;j++)
      for(int i=imin;i<imax;i++) {
        ...
        Avec[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,0)] = AxL;
        ...
        //Fill in the interface variables:
        vel[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,0)] = ETVx; //0.0;
        vel[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,1)] = ETVy; //0.0;
        vel[CCTK_VECTGFINDEX3D(cctkGH,i,j,k,2)] = ETVz; //0.0;
