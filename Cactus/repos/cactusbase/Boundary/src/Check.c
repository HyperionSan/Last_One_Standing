...
void BndSanityCheckWidths(const cGH *GH, CCTK_INT varindex, CCTK_INT dim,
  ...
  for (int i = 0; i < 2 * dim; i++) {
    ...
    if (boundary_widths[i] > 100 || boundary_widths[i] < 0) {
      ...
      const char fmt[] = "x[%d]"; // use when dim is not x,y,z
      char dir[sizeof(dim)*3 + sizeof(fmt)]; // enough space for 3 digits per byte
      if(dim >= 1 && dim <= 3) {
        snprintf(dir, sizeof(dir), "%c", "xyz"[dim-1]);
      } else {
        snprintf(dir, sizeof(dir), fmt, dim);
      }
      CCTK_VError(__LINE__, __FILE__, CCTK_THORNSTRING,
                  "Tried to register a physical boundary condition \"%s\" for "
                  "variable \"%s\" with a boundary zone width %d (%s) in the %c%s "
                  "direction -- this looks like an error.",
                  bcname, fullname, boundary_widths[i], cond, "-+"[dim%2], dir);
      free(fullname);
    }
  }
}
