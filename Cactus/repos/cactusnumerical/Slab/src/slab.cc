...
static MPI_Datatype MPI_LONG_LONG;
...
// this is the only one (?) actually used of the MPI replacements
static int MPI_Alltoallv(void *sendbuf, ptrdiff_t *sendcnt, ptrdiff_t *sendoff,
                         int sendtype, void *recvbuf, ptrdiff_t *recvcnt,
                         ptrdiff_t *recvoff, int recvtype, MPI_Comm comm) {
  int recvsize;
  assert(sendbuf);
  assert(recvbuf);
  assert(sendcnt);
...
#ifdef HAVE_CCTK_INT1
  case sizeof(CCTK_INT1):
    MPI_LONG_LONG = CACTUS_MPI_INT1;
    break;
#endif
#ifdef HAVE_CCTK_INT2
  case sizeof(CCTK_INT2):
    MPI_LONG_LONG = CACTUS_MPI_INT2;
    break;
#endif
#ifdef HAVE_CCTK_INT4
  case sizeof(CCTK_INT4):
    MPI_LONG_LONG = CACTUS_MPI_INT4;
    break;
#endif
#ifdef HAVE_CCTK_INT8
  case sizeof(CCTK_INT8):
    MPI_LONG_LONG = CACTUS_MPI_INT8;
    break;
#endif
  default:
    CCTK_BUILTIN_UNREACHABLE();
...
Slab_MultiTransfer_Apply(cGH const *restrict const cctkGH,
  ...
  for (int vartypei = 0; vartypei < nvartypes; ++vartypei) {
    ...
    ifcheck {
      vector<long long> src1count(size);
      vector<long long> dst1count(size);
      vector<long long> src2count(size);
      vector<long long> dst2count(size);
      ifdebug fflush(stdout);
      for (int n = 0; n < size; ++n) {
        src1count[n] = dstcount[n];
        dst1count[n] = srccount[n];
      }
      ...
      MPI_Alltoall(&dstcount.front(), 1, MPI_LONG_LONG, &dst2count.front(), 1,
                   MPI_LONG_LONG, comm);
      for (int n = 0; n < size; ++n) {
        check(src2count[n] == dst1count[n]);
        check(dst2count[n] == src1count[n]);
      }
    }
    ...
    if (use_alltoallv) {
#ifdef HAVE_CAPABILITY_MPI
      // actual MPI_Alltoallv does not support 64-bit integers
      ...
      // Wait (if needed and we have MPI)
