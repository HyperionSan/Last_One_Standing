...
void VolumeIntegrals_vacuum_ComputeIntegrand(CCTK_ARGUMENTS)
{
  ...
  if(CCTK_EQUALS(Integration_quantity_keyword[which_integral],"H_M_CnstraintsL2")) {
    ...
    const CCTK_REAL *restrict H_gf   = (const CCTK_REAL *restrict)(CCTK_VarDataPtr(cctkGH,timelevel, HamiltonianVarString));
    ...
    const CCTK_REAL *restrict MU0_gf = (const CCTK_REAL *restrict)(CCTK_VarDataPtr(cctkGH,timelevel, Momentum0VarString));;
    ...
    const CCTK_REAL *restrict MU1_gf = (const CCTK_REAL *restrict)(CCTK_VarDataPtr(cctkGH,timelevel, Momentum1VarString));;
    ...
    const CCTK_REAL *restrict MU2_gf = (const CCTK_REAL *restrict)(CCTK_VarDataPtr(cctkGH,timelevel, Momentum2VarString));;
...
#pragma omp parallel for collapse(3)
  ...
  } else if(CCTK_EQUALS(Integration_quantity_keyword[which_integral],"H_M2_CnstraintsL2")) {
    // Get pointer to Hamiltonian constraint gridfunction
    const CCTK_REAL *restrict H_gf   = (const CCTK_REAL *restrict)(CCTK_VarDataPtr(cctkGH,timelevel, HamiltonianVarString));
    if(  !H_gf  ) CCTK_VError (__LINE__, __FILE__, CCTK_THORNSTRING, "Couldn't get data pointer of input array variable '%s'", HamiltonianVarString);
    // Get pointer to Momentum constraint-squared gridfunction M^i M_i
    const CCTK_REAL *restrict M2_gf = (const CCTK_REAL *restrict)(CCTK_VarDataPtr(cctkGH,timelevel, MomentumSquaredVarString));;
    if( !M2_gf ) CCTK_VError (__LINE__, __FILE__, CCTK_THORNSTRING, "Couldn't get data pointer of input array variable '%s'", MomentumSquaredVarString);
#pragma omp parallel for collapse(3)
    for (int k=0;k<cctk_lsh[2];k++) for (int j=0;j<cctk_lsh[1];j++) for (int i=0;i<cctk_lsh[0];i++) { int index = CCTK_GFINDEX3D(cctkGH,i,j,k);
	  VolumeIntegrals_vacuum_L2_integrand(VolIntegrand1, index,H_gf,x,y,z);
	  VolumeIntegrals_vacuum_L2_integrand(VolIntegrand2, index,M2_gf,x,y,z); }
